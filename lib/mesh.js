...
function Mesh(config) {
  ...
  Object.defineProperty(this, 'tools', {
    value: {}
    // mesh.tools.widgets uses this. (affects system/components/api/index, system/widgets)
  });
...
Mesh.prototype.initialize = Promise.promisify(function(config, callback){
  ...
  configer.process(this, config, function(e, config) {
    ...
    if (Object.keys(root.nodes).length == 1) { // i believe that with node it is impossible
                                              // for this to get from 0 to 2 without passing
                                             // through 1, is that the case?
  ...
  this.__stopComponents(function(e){
    ...
    if (e){
      ...
      if (timeout) {
        ...
        _this._mesh.datalayer.server.stop(function(e) {
...
Mesh.prototype._registerSchema = function(config, callback){
  ...
  this.data.set('/mesh/schema/description', description, null, function(e, response){
    if (e) return callback(e);
